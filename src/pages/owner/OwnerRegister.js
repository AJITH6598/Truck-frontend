...
import { ownerSendOTP, ownerRegister } from '../../utils/api';
import { FaTruck, FaArrowLeft, FaExclamationTriangle, FaCheckCircle, FaEye, FaEyeSlash, FaClock } from 'react-icons/fa';
import ownerLogImg from '../../images/owner.log.png';

const OwnerRegister = () => {
  const [form, setForm] = useState({
    transportName: '', username: '', mobile: '', email: '',
  ...
  const handleChange = (e) => {
    ...
    let newValue = value;
    if (name === 'transportName' || name === 'username') {
  ...
  const handleSendOTP = async () => {
    ...
    setOtpLoading(true);
    try {
      await ownerSendOTP({ email: form.email, username: form.username });
      setOtpSent(true);
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="auth-card" style={{ maxWidth: 500 }}>
        <Link to="/owner/login" className="back-link"><FaArrowLeft style={{ marginRight: 8 }} /> Back to Login</Link>
        ...
        <form onSubmit={handleSubmit}>
          {[
            { label: 'Transport Name', name: 'transportName', placeholder: 'Enter transport name' },
